...
\item Here is a web page of 40,000 sampled words
  ...
  \item Compare this to the $\alpha$ that minimizes
    ...
    The first and second derivative of (\ref{eq:prob3c:f}) are:
    \begin{align*}
      \df{d}{da}\left(\sum_{x=0}^{26} \left| p(x) -  q(x) \right|^2\right)
      & = \sum_{x=0}^{26} \left( (2x+2)a^{2x+1} - (2x+2)a^{2x} + 2xa^{2x-1} + (x+1)\df{2C_xB_x}{B} a^{x} \right.\\
      & \quad \quad \quad \quad \left.  - x\df{2C_xB_x}{B} a^{x-1}\right)\\
      \df{d^2}{da^2}\left(\sum_{x=0}^{26} \left| p(x) -  q(x) \right|^2\right)
      & = \sum_{x=0}^{26} \left( (2x+2)(2x+1)a^{2x} - (2x+2)2xa^{2x-1} + 2x(2x-1)a^{2x-2} + \right. \\
      & \quad \quad \quad \quad \left. x(x+1)\df{2C_xB_x}{B} a^{x-1} - x(x-1)\df{2C_xB_x}{B} a^{x-2}\right)
    \end{align*}
    \begin{align*}
